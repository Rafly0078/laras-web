 ...
 *
 *  Asal    : src/utils/Lyrics/ttml/parser.ts
 ...
 *
 *  DIMODIFIKASI oleh proyek LARAS pada 2026-09-02: hanya impor Logger yang
 *  ditukar ke shim lokal; seluruh logika parsing tetap verbatim.
 *
 ...
 *  berguna, TANPA JAMINAN APA PUN. Salinan lisensinya ada di file `LICENSE`
 *  di akar repo ini (sudah AGPL-3.0).
 *
 *  Setiap penyesuaian LARAS di bawah ini ditandai komentar `// LARAS: ...`.
 *  Jangan "rapikan" file ini — makin dekat ke upstream, makin murah menarik
 *  perbaikan mereka berikutnya.
 * ═══════════════════════════════════════════════════════════════════════════
 */

import { XMLParser } from 'fast-xml-parser';

// LARAS: Logger upstream menarik Maid + nanostores + config Spicetify; di Node
// (unit test) dan di server Next semua itu tidak ada. Shim lokal berantarmuka
// sama supaya badan parser tidak perlu disentuh.
    ...
    case 'Syllable': {
      ...
      divs.forEach((div) => {
        ...
        ps.forEach((p) => {
          ...
          // LARAS: upstream memakai `strictNullChecks: false`; di sini `strict`
          // menyala, dan penyempitan `ttml.tt.body` di atas tidak bertahan
          // masuk ke dalam callback. Optional chaining, semantik tak berubah
          // (kedua cabang sebelumnya sudah dijaga guard di awal convertTTML).
